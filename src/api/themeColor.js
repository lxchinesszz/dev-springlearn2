...
const getMedianColor = (colorCountMap, total) => {
    ...
    for (let i = 0; i <= medianIndex; i += 1) {
        medianCount += sortArr[i].count;
    }
    return {
        color: parseInt(sortArr[medianIndex].color, 10),
        count: medianCount,
    };
};
// 切割颜色盒子
const cutBox = (colorBox) => {
    const { colorRange } = colorBox;
    const cutSide = getCutSide(colorRange);
    const colorCountMap = {};
    const { total } = colorBox;
    const { data } = colorBox;
    // 统计出各个值的数量
    for (let i = 0; i < total; i += 1) {
...
const colorFilter = (colorArr, difference) => {
    for (let i = 0; i < colorArr.length; i += 1) {
        for (let j = i + 1; j < colorArr.length; j += 1) {
            if (Math.abs(colorArr[i][0] - colorArr[j][0]) < difference &&
...
const themeColor = (colorNumber, img, difference, callback) => {
    ...
    let colorArr = [];
    for (let j = 0; j < colorBoxArr.length; j += 1) {
